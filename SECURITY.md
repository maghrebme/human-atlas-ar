...
الإبلاغ الخاص عن الثغرات مفعّل. استخدم **Security → Report a vulnerability** لإرسال تقرير سري يتضمن خطوات إعادة المشكلة وتأثيرها. لا يتطلب التقرير نشر بريدك في الشيفرة. إذا لم يظهر الخيار، لا تضع تفاصيل الثغرة في قضية عامة؛ اطلب تفعيل الإبلاغ الخاص دون تفاصيل حساسة.

Security fixes target `main`. Private vulnerability reporting is enabled. Use **Security → Report a vulnerability**. Include reproduction steps, affected versions and impact; do not include live credentials or personal datasets. If reporting is unavailable, request its activation without disclosing exploit details publicly. No response-time guarantee is offered.
