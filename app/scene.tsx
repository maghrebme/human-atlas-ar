import {explosionTarget} from './explosion-camera';
import {bilingualName} from './arabic';
...
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError}:Props){
 ...
 useEffect(()=>{
  ...
  try{renderer=new T.WebGLRenderer({antialias:true,alpha:false,powerPreference:'high-performance'});}catch{onError('تعذر تشغيل العارض ثلاثي الأبعاد. استخدم متصفحًا يدعم WebGL.');return;}
  ...
  renderer.domElement.setAttribute('aria-label','تشريح إنسان تفاعلي. اسحب للتدوير، وباعد إصبعيك أو مرر للتكبير، وانقر على بنية لاستكشافها.');
  ...
  const loadChunk=async(ci:number)=>{
   ...
   groups.forEach((gs,system)=>{const geometry=mergeGeometries(gs,false);if(!geometry)throw new Error('تعذر تجميع هندسة التشريح.');geometries.push(geometry);const mesh=new T.Mesh(geometry,mats.get(system as never));mesh.frustumCulled=false;scene.add(mesh);});
   lastState=null;loaded++;onProgress(Math.round(loaded/atlas.chunks.length*100));dirty=true;
  };
  (async()=>{try{let cursor=0;await Promise.all(Array.from({length:3},async()=>{while(cursor<atlas.chunks.length){const i=cursor++;await loadChunk(i);}}));if(!disposed){ready=true;dirty=true;}}catch(e){if(!disposed)onError('تعذر تحميل التشريح. تحقق من الاتصال ثم أعد تحميل العارض.');}})();
  const fit=(view:string,extent=0)=>{
   ...
   const target=explosionTarget(extent,packingWidth,mobile);controls.target.set(target.x,target.y,0);camera.position.copy(controls.target).addScaledVector(direction,distance);controls.update();dirty=true;
  };
  ...
  const contextLost=(e:Event)=>{e.preventDefault();onError('أوقف الجهاز جلسة العرض ثلاثي الأبعاد. أعد التحميل للمتابعة.');};renderer.domElement.addEventListener('webglcontextlost',contextLost);
